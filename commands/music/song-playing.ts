...
export default {
  ...
  description: { en: "Display the currently playing song", th: "แสดงเพลงที่กำลังเล่นอยู่" },
  ...
  execute: async (ctx) => {
    ...
    await ctx.reply(`${t.song.songPlaying(current.title, current.author, current.requestedBy)} (${queueStatus})`);
  },
} satisfies Command;
