...
export default {
  ...
  arguments: [
    {
      ...
      description: { en: "The user you want to give money", th: "ผู้ใช้ที่คุณต้องการให้เงิน" },
      required: true,
    },
    {
      name: { en: "amount", th: "จำนวนเงิน" },
      description: { en: "The amount of money you want to give", th: "จำนวนเงินที่คุณต้องการให้" },
      required: true,
    },
  ],
  ...
  execute: async (ctx, args) => {
    ...
    ctx.emit("lookupUser", { platform: ctx.user.platform, name: targetName, callback: async (targetId: string | null) => {
      ...
      ctx.emit("feed", { status: "normal", icon: "📩", name: `${ctx.user.name} ➡ ${targetName}`, action: `${amount} ${ctx.currency}` });
